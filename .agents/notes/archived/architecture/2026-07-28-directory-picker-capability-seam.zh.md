...
- **native 后端保留。** 插件化正是目的：多个提供方都能提供该 seam（原生壳可以经自己的对话框 API 提供 `native` 交互）。kind 命名：最初选了 `dialog` 后被放弃——browse 交互同样以对话框呈现（应用内弹窗），这个词起不到判别作用；`native` 命名的是选择器运行的位置。
...
- 未来的新交互（或提供 `native` 交互的原生提供方）只是一个双面后端包——无需网关手术，也不动 ui-workspace。
