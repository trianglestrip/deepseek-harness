...
- `area/web` 覆盖浏览器与桌面壳图形界面，`area/vscode` 覆盖编辑器扩展，`area/api` 覆盖跨界面协议与各语言 SDK。
...
**为每种交付载体或媒体生命周期单设标签。** 浏览器与桌面壳交付共用一个图形界面领域，产物、附件与多模态交付目前也共用一个评审／查询领域。只有当拆分能恢复有用的独立分类时，才应在后续分类体系变更中进行。
