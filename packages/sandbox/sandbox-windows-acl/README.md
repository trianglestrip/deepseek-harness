...
- **Confined children start without a console window** — the runner creates every child with `CREATE_NO_WINDOW`, so a host with no console of its own (the packaged desktop application) gets no console window per command; the child runs in a windowless console and mirrors its exit code, and pipe-based stdio redirection is unaffected.
