/**
 * Desktop Host entry: boots the desktop project without a listening socket and
 * carries API plus validated Web assets over framed byte pipes.
...
import { createCarrier } from './carriers.ts'
import { runDesktopHostSession } from './session.ts'

export { DESKTOP_HOST_PROTOCOL_VERSION } from './wire.ts'
export type { DesktopHostLifecycleEvent } from './wire.ts'
export type { DesktopHostTransport } from './carriers.ts'

/** One request forwarded from the shell's application protocol handler. */
...
const ROOT_CONFIG = '# Desktop composition root; package transactions own this file.\n[]\n'
...
/**
 * Renderer assets and index injections.
 * @param ctx - active composition supplying plugin index rows.
 * @param runtimeDir - immutable dsh packages carried by the application.
 * @param indexScript - shell-supplied transport script; absent when the shell injects its own.
 * @returns the buffered asset handler served over the carrier.
 */
function assetHandler(ctx: Context, runtimeDir: string, indexScript: string | undefined): ConnectionFetchHandler {
  ...
  const renderIndex = async (): Promise<Response> => {
    const rows: IndexInjection[] = indexScript === undefined
      ? []
      : [{ kind: 'script', placement: 'head', text: indexScript }]
 ...
 * @param runtimeDir - immutable dsh packages supplied by the application.
 * @param projectDir - active or staged shell-owned desktop profile.
 * @param writeResponse - serialized response writer that applies byte backpressure.
 * @param options - development-only allowance for workspace-linked bundle packages, plus the
 * shell's renderer transport script.
...
export async function runDesktopHost(
  ...
  options: { allowLinkedPackages?: boolean; indexScript?: string } = {},
  ...
  const assets = assetHandler(ctx, resolve(runtimeDir), options.indexScript)
...
/**
 * Serve one Host process for a desktop shell.
 *
 * The transport comes from `DSH_DESKTOP_TRANSPORT` and the shell's renderer
 * transport script from `DSH_DESKTOP_TRANSPORT_SCRIPT`, defaulting to the script
 * the Electron shell expects.
 */
async function main(): Promise<void> {
  const runtimeDir = process.argv[2]
  const projectDir = process.argv[3]
  if (runtimeDir === undefined || projectDir === undefined) {
    throw new Error('dsh desktop: expected a runtime directory and a profile directory')
  }
  ...
  const indexScript = readTransportScript()
  const controller = await runDesktopHost(runtimeDir, projectDir, frame => carrier.write(frame), {
    allowLinkedPackages: option !== undefined,
    ...(indexScript === undefined ? {} : { indexScript }),
  })
  await runDesktopHostSession(controller, carrier)
  // Standard streams stay open after the session, so the `stdio` carrier ends
  // the process explicitly; the `fd` carrier exits when its descriptors close.
  if (carrier.transport === 'stdio') process.exit(process.exitCode ?? 0)
}

/** The shell's renderer transport script, present when it ships one. */
function readTransportScript(): string | undefined {
  const path = process.env.DSH_DESKTOP_TRANSPORT_SCRIPT
  if (path === undefined || path === '') return DESKTOP_TRANSPORT_SCRIPT
...
if (import.meta.main) {
  main().catch((error: unknown) => {
    ...
    // The Electron shell learns about startup failure on the IPC channel.
    if ((process.env.DSH_DESKTOP_TRANSPORT ?? 'fd') === 'fd') process.send?.({ type: 'fatal', message })
    process.exitCode = 1
  })
}
